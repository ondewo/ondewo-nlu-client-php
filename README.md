...
[ondewo-nlu-api](https://github.com/ondewo/ondewo-nlu-api) repository by the
...
// `package ondewo.nlu;` becomes `Ondewo\Nlu`, and a service `Agents` becomes
// `AgentsClient` (grpc_php_plugin's default class suffix). Browse src/Ondewo/Nlu for
...
$client = new AgentsClient(
    getenv('ONDEWO_NLU_HOST') ?: 'localhost:50055',
    $auth->channelOptions(ChannelCredentials::createSsl())
);

$request = new ListAgentsRequest();
$request->setAgentView(AgentView::AGENT_VIEW_SHALLOW);

[$response, $status] = $client->ListAgents($request)->wait();
