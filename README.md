...
The client is published on [Packagist](https://packagist.org/packages/ondewo/nlu-client-php) as
...
The client version tracks the ONDEWO NLU API in major and minor version, so pin the minor series that
...
* **`ext-grpc` is a hard requirement**, not an optional extra — every generated `<Service>Client`
  ...
  (see [Requirements](#requirements) above); `composer require ondewo/nlu-client-php --ignore-platform-req=ext-grpc`
...
* **There is no build step.** Packagist serves the tree of a git tag verbatim and the generated stubs are
  ...
  `composer require`, `vendor/ondewo/nlu-client-php/src/` already holds every stub and the autoloader is
  generated by composer itself. Nothing needs docker, protoc or the `ondewo-nlu-api` submodule.
...
use Ondewo\Nlu\AgentsClient;                    // generated, from src/
use Ondewo\Nlu\Auth\BearerTokenAuthenticator;   // hand-written, from auth/
```
