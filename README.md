...
// `package ondewo.nlu;` becomes `Ondewo\Nlu`, and a service `Foo` becomes
// `FooClient` (grpc_php_plugin's default class suffix). Browse src/Ondewo/Nlu for
// the services and messages your pinned API version actually declares.
$client = new \Ondewo\Nlu\ExampleServiceClient(
...
$request = new \Ondewo\Nlu\ExampleRequest();
