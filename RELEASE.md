...
* Published to [Packagist](https://packagist.org/packages/ondewo/nlu-client-php) as `ondewo/nlu-client-php`.
...
* `make packagist_dry_run` is the credential-free half of that path and runs in CI on every push:
  ...
  agreement between `ONDEWO_NLU_VERSION`, `composer.json`, `RELEASE.md` and the git tag, and the exact update
